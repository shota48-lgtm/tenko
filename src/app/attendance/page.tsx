...
export default function AttendancePage() {
  ...
  return (
    <main className="min-h-screen" style={{ background: "var(--tk-paper)" }}>
      ...
      <p className="mt-1 text-xs tk-soft">
        自動では直しません。内容を見て、必要なら下の修正申請を出してください。
        {thresholds && `（長時間の閾値 ${thresholds.longWorkHours} 時間 / 直近 ${thresholds.days} 日を対象）`}
      </p>
      {anomalies.length === 0 && (
        <p className="tk-panel mt-2 px-3 py-5 text-center text-sm">確認が必要な記録はありません。</p>
      )}
      <ul className="tk-panel mt-2 text-sm">
        {anomalies.map((a, i) => (
          <li key={i} className="tk-sep px-3 py-2">
            <strong>{a.workDate}</strong> {a.displayName} ／ {labels[a.kind] ?? a.kind}
            <div className="text-xs tk-soft">{a.detail}</div>
          </li>
        ))}
      </ul>
      ...
      <div className="tk-panel mt-2 flex flex-wrap items-center gap-2 p-2">
        <select value={kind} onChange={(e) => setKind(e.target.value)} className="tk-input text-xs">
          {["arrive", "leave", "break", "late"].map((k) => <option key={k} value={k}>{KIND_LABEL[k]}</option>)}
        </select>{" "}
        <input type="datetime-local" value={when} onChange={(e) => setWhen(e.target.value)} className="tk-input text-xs" />
        <input placeholder="直す記録のID（押し忘れの追加なら空欄）" value={target}
          onChange={(e) => setTarget(e.target.value)} className="w-56 tk-input text-xs" />
        <input placeholder="理由" value={reason} onChange={(e) => setReason(e.target.value)} className="w-64 tk-input text-xs" />
        <button onClick={submit} className="tk-btn text-xs">申請</button>
      </div>

      <h2 className="mt-6 text-sm font-semibold">申請の状況（{corrections.length} 件）</h2>
      <ul className="tk-panel mt-2 text-sm">
        {corrections.map((c) => (
          <li key={c.id} className="tk-sep px-3 py-2">
            ...
            <div className="text-xs tk-soft">理由: {c.correction_reason}</div>
