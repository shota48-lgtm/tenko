...
### 地域を変えたあとに、守りが崩れていないかを流し直した

デプロイの地域を変えたので、本番に対する破壊試験をもう一度全部流した。

```
attack-public-view : OK 24 件 / 通ってしまった 0 件
attack-phase5-auth : OK 45 件 / 通ってしまった 0 件
attack-presence(S6): OK 12 件 / 通ってしまった 0 件
attack-ws-verify   : OK 4 件 / 通ってしまった 0 件
attack-origin      : OK 5 件 / 通ってしまった 0 件
verify-freeze      : 通ってしまった 0 件
```

**すべて維持されている。** `assertSameOrigin` も地域が変わった後で 403 を返している。

### セッション確認の遅延についての意見（**下の意見は、地域を直す前のものである。追記2で取り下げた**）
...
### 追記（同日）: 遅い理由が分かった。**地域が揃っていなかった**

POが Vercel の Function Region を `sin1`（シンガポール）に変えて再デプロイした。
**測り直したが、数字は1msも変わらなかった。**

```
A: /api/me（Cookie なし・DBを引かない）      中央 204 ms   （変更前 200 ms）
B: /api/me（Cookie あり・DBを3クエリ）        中央 1075 ms  （変更前 1077 ms）
B - A                                        約 871 ms     （変更前 862 ms）
```

**設定が効いていないのではないか**と考え、関数が実際にどこで動いているかを応答ヘッダで確かめた。

```
/api/rooms          X-Vercel-Id: hnd1::iad1::nvx7h-...
/api/users          X-Vercel-Id: hnd1::iad1::mbhn8-...
/api/demo-presence  X-Vercel-Id: hnd1::iad1::9lqdn-...
/                   X-Vercel-Id: hnd1::iad1::hpdj4-...
```

左が要求を受けた edge（`hnd1` = 東京）、**右が関数が実際に動いた地域（`iad1` = 米国東部）**。

**まだ米国東部で動いている。** DBはシンガポール（`ap-southeast-1`）にある。

裏づけとして、手元（日本）から Neon への往復も測った:

| 経路 | 1クエリの往復 |
|---|---|
| 手元（日本）→ Neon（シンガポール） | **76 ms** |
| Vercel（**米国東部**）→ Neon（シンガポール） | **約 213〜290 ms** |

**米国東部とシンガポールの往復は 230ms 前後。実測とぴたり合う。**
「地域が離れている」という推論は正しく、**まだ揃っていないだけだった。**

対処: `vercel.json` に `{"regions": ["sin1"]}` を置いた。
**画面の設定より、リポジトリに入る設定のほうが確実に効く**（デプロイのたびに同じ地域になる）。

### 追記2（同日）: `vercel.json` が効いた。**DBの分が 862ms → 11ms になった**

POが `vercel.json` を push し、Neon のパスワードもローテーションした（接続文字列がチャットに出たため）。
関数が実際にシンガポールで動いていることを、まずヘッダで確かめた。

```
/api/rooms   X-Vercel-Id: hnd1::sin1::ps9wl-...
/api/users   X-Vercel-Id: hnd1::sin1::cr42x-...
/            X-Vercel-Id: hnd1::sin1::hlqw7-...
                     ↑ iad1（米国東部）から sin1（シンガポール）に変わった
```

測り直した結果:

| | **iad1（米国東部）** | **sin1（シンガポール）** | 変化 |
|---|---|---|---|
| A: DBを引かない分 | 204 ms | **113 ms** | −45% |
| B: `/api/me`（DBを3クエリ） | 1075 ms | **124 ms** | **−88%** |
| **B − A（DBの3クエリ分）** | **862 ms** | **11 ms** | **78分の1** |
| `/api/rooms`（DBを1クエリ） | 417 ms | 110 ms | −74% |
| 村が出るまで（最初のHTML） | 648 ms | **127 ms** | −80% |

**1クエリあたり約 3.7 ms。** 同じ地域どうしの往復として妥当な数字である。

### **設計書の見積は正しかった。外れていたのは自分の読み方だった**

設計書5節はこう書いていた。

> **Vercel と Neon が同じ地域なら**、往復は概ね 1〜3ms/クエリ。1リクエストあたり 2〜10ms 程度と見積もる

**実測は 11ms。見積の範囲にほぼ収まっている。**

段階7-B の最初、1077ms を測ったときに「**設計書の見積は2桁外れていた**」と書いた。**これは誤りだった。**
見積には「**同じ地域なら**」という条件が付いていて、**その条件が満たされているかを確かめずに、
数字だけを見て見積を誤りと判定していた。**

正しい順序は「見積が外れた → **前提が成り立っているかをまず確かめる**」であり、
今回それをやったから原因（地域が違う）に辿り着いた。
**先に「見積が悪い」と結論づけていたら、3クエリを1クエリに減らす作業に進み、
本当の原因（地域）を残したまま設計を歪めていた。**

### 3クエリの構造について（**当初の意見を取り下げる**）

段階7-B の最初に「役割を毎回引き直す判断を見直す必要があるかもしれない」と書いたが、
**その必要は無い。** DBの3クエリで 11ms である。

**「降格が即座に効く」という担保を、11ms で買えている。** これは安い。
...
## G-2. Render の停止・起動（**測れた**）

POが全てのタブを閉じ、`/healthz` が `ok 0` になった状態から測った。

**16.6分間、Render に一切触れずに待った**（`/healthz` を叩くだけでも起きてしまうため、
待っている間は Vercel 側だけを触った）。

```
16:18:55  healthz: ok 0     ← ここから触らない
（16.6分）
16:35:42  測定開始
```
...
**ws-server に一切触れずに**、アプリ（Vercel）から村のHTMLだけを取った。
...
**ws-server が停止していても、村には20人いる。**
無料枠を選んだ判断の根拠が、実測で保たれていることを確認した。

画面でも同じことを確かめた（もう一度16分待って、停止させてから開いた）。

| | メンバー | 自分のアバター |
|---|---|---|
| **開いた直後（ws-server は停止中）** | **20/56** | **出ていない** |
| 1分後（起動して繋がったあと） | 21/56 | 「あなた」の枠つきで出る |

**停止中でも、村・建物・20人・吹き出し・状態のリングはすべて描かれている。**
違いは「自分が村にいるかどうか」だけである。
（採用担当が開いた瞬間に見えるのは、この20人のいる村である）

### 起動にかかる実時間 → **12.5 秒**（公表値は「約1分」）

```
=== 2. ws-server を起こす（WebSocket の接続）
  **起動までの実測: 12.5 秒**
  最初の presence.list: 20 人（12.5 秒）
```

**WebSocket の接続だけで起きる**（HTTPを叩く必要がない）。
これは無料枠の前提そのもの（誰かが村を開けば起きる）なので、確かめる価値があった。

### 本当に停止していたことの裏づけ（対照）

同じ測定を、起きている状態でもう一度行った。

| | 起動までの時間 |
|---|---|
| **16.6分放置したあと（冷えている）** | **12.5 秒** |
| その直後（起きている） | **0.3 秒** |

**40倍の差。** 12.5秒が「起動にかかった時間」であることが、この対照で確かめられる。
`/healthz` も起きている状態では 0.48 秒で返る。

### 誰かが村にいる間は停止しないか → **しない**

段階7-B の前半で、**接続が2本ある状態のまま75分間、停止しなかった**ことを確認している。

```
13:46:34  ok 2
14:02:35  ok 2   ← 起動（12:47）から75分、15分の無通信ルールにかかわらず停止していない
```

**WebSocket が繋がっている限り止まらない**（2026年2月の変更どおり）。

### ws-server の外向きの通信は停止を妨げるか → **妨げない**

ws-server は `/api/rooms` と `/api/demo-presence` を60秒ごとに叩いている。
**この状態で停止した**（上の実測）。停止の判定は**受信**の有無で行われ、
送信は数えられていないことが、実測で確かめられた。
...
| 事項 | 区分 | 内容 |
|---|---|---|
| 15分で実際に停止するか | **実測済み** | 16.6分の放置で停止した（G-2節） |
| 停止中に村が見えるか | **実測済み** | ws-server に触れずにHTMLを取り、**20人が載っている**ことを確認（G-2節） |
| 起動にかかる実時間 | **実測済み** | **12.5秒**（起きている状態の 0.3 秒との対照つき）。公表値の「約1分」より速い |
| ws-server の外向きの通信が停止を妨げるか | **実測済み** | 60秒ごとに送信していても停止した。**受信だけが数えられている** |
| 関数の実行地域を sin1 にできるか | **実測済み** | `vercel.json` で効いた。`X-Vercel-Id` が `sin1` になり、DBの分が 862ms→11ms |
| Vercel の無料枠の実際の上限 | **未確認** | 管理画面はPOの職掌 |
| セッション確認の遅さ | **解決** | 地域を揃えて 119ms。**判断を仰ぐ必要は無くなった**（追記2） |
| デモ用の利用者が動かないこと | **未解決（判断待ち）** | 段階4-5からの持ち越し |
