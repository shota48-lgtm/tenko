...
> **この節は 2026-08-13 に、スパイク `spike/ws-auth-01` の実測にもとづいて書き直した。**
> 方式（入場券を `Sec-WebSocket-Protocol` で渡す）は変えていない。
> 変えたのは**符号化の要件・断り方・応答に券を返さない理由**の3点で、
> いずれも書き直す前の記述が実測と食い違っていた。詳細は11節の変更ログに残す。
...
[3] ws-server が handleProtocols で券を取り出し、検証する
      → 鍵は TENKO_WS_TICKET_SECRET。アプリと共有する
      → nonce を10分間だけ記憶し、使い回しを拒む
      → **判定の結果を接続に覚えさせるだけにする。ここでは断らない**
      → 応答の Sec-WebSocket-Protocol には "tenko.v1" だけを返す（券は返さない）
[4] connection で、覚えておいた判定を見る
      → 認証済み: 以後、接続に紐づくIDだけを使う
      → 不正な券: close(4003)
      → 券なし  : 「見るだけ」として受ける（TENKO_PUBLIC_VIEW=0 なら close(4004)）
```

### 符号化は base64url。**パディング（`=`）を付けないこと**

`Sec-WebSocket-Protocol` の値は RFC 6455 のトークンであり、`=` `+` `/` は使えない。

**`=` が1文字でも入ると、ブラウザは `new WebSocket` の時点で例外を投げ、接続を試みもしない。**

```
SyntaxError: Failed to construct 'WebSocket':
The subprotocol 'ticket.MS5zZXNz…YmE=.30zphthMjpyw1BwforCddijBV1ynRO3odv/h7Kmiyto=' is invalid.
```

このときサーバには**何も届かない**（サーバのログに記録が残らない）。
つまり「サーバ側で寛容に受ける」対策は取れない。**発行する側で正しく符号化するしかない。**

Node では `Buffer.toString("base64url")` がこの形（パディングなし・`-` と `_`）になる。
検証側は `Buffer.from(s, "base64url")` で戻せる。

券の長さは 160 文字前後（`ticket.` と `tenko.v1` を足したヘッダ全長で約 180 文字）。
実測では 8007 文字まで通り、16007 文字で失敗した（`onclose code=1006`、サーバのハンドラは呼ばれない。
HTTPヘッダの上限に当たったものと考えられる＝**推論**）。**上限に対して十分な余裕がある。**

### 不正な券は、**ハンドシェイクで断らない**

`handleProtocols` が `false` を返す形にしてはいけない。理由は2つある。

**理由1: close code が画面に届かない。**

| やり方 | ブラウザの `onclose` |
|---|---|
| `handleProtocols` で `false` を返す | `code=1006 reason="" wasClean=false` |
| いったん受け入れ、直後に `ws.close(4003, "ticket expired")` | **`code=4003 reason="ticket expired" wasClean=true`** |

1006 は「異常終了」を表すだけで、**「券を取り直せば入れる」のか「サーバが落ちている」のかを
画面が区別できない**。区別できないと、券を取り直すべき場面で取り直せない。

**理由2: `false` を返しても、サーバ側では接続が発生する。**

```
判定: 不可（期限切れ）
[connection] authed=false (期限切れ) 選ばれた protocol=""   ← 接続は張られている
[close] code=1006
```

`ws` は「応答に `Sec-WebSocket-Protocol` を付けない」だけで、接続自体は成立させる。
**断っているのはブラウザの側**である。`false` を「拒否」と思って書くと、
**拒否したはずの接続がサーバ側に存在する**ことになる。

したがって:

```
handleProtocols → 券を検証し、結果を接続に覚えさせるだけ。応答は常に "tenko.v1"
connection      → 覚えた結果を見て、不正なら close(4003) / close(4004)
message         → **先頭で authed を見て、偽なら種別を問わず捨てる**
```

`message` の判定が必要なのは、閉じるまでの一瞬に送られうるためである。
実測で、認証前の接続から送ったメッセージが捨てられることを確認した。

### 応答に券を返さない理由

**券を応答に含めても接続は成立する**（実測。`ws.protocol` が券の文字列になるだけ）。
それでも `tenko.v1` だけを返すのは、**券を応答ヘッダに載せないため**である。
券をURLに載せない理由（経路のログに残る）と同じ理由が、応答側にも当てはまる。

一方、**何も返さない**（`false`）と、ブラウザは 1006 で切る。**応答は必ず返すこと。**
...
| **`Sec-WebSocket-Protocol`** | ブラウザから唯一送れるヘッダ。**URLに載らない**ためログに残りにくい | 本来の用途（サブプロトコル交渉）ではない転用。**応答を返さないと 1006 で切れる** | **採用** |
| 接続後の最初のメッセージ | 素直 | 認証前の接続が存在する時間ができ、タイムアウトの管理が要る | 不採用 |
...
| 案 | 評価 |
|---|---|
| **A（採用）: 引かない。** 入場時はHMACの検証のみ。無効化の確認は、アプリの `/api/ws-verify` へ定期問い合わせ | ws-server に DB の資格情報を置かなくて済む（漏洩面が1つ減る） |
| B: ws-server が Neon を直接引く | 無効化を即座に判定できる。ただし ws-server に `DATABASE_URL` が要り、Railway 側にDBの鍵を置くことになる |

定期問い合わせの形:

```
ws-server → POST /api/ws-verify  { sessionIds: [...] }   ヘッダ: x-tenko-internal: <共有秘密>
アプリ    → { valid: [...], invalid: [...] }
...
**`/api/demo-presence` とは1本にまとめない**（2026-08-13 PO判断）。
周期は同じでも、失敗したときの扱いが逆であるため。

| 経路 | 取得に失敗したとき |
|---|---|
| `/api/demo-presence` | **前回の内容を使い続ける**（村が空にならないように） |
| `/api/ws-verify` | **段階的に切断する**（無効化の仕組みが死んだままにならないように） |

まとめると、片方の障害がもう片方を巻き込む。「同じ周期だから」はまとめる理由にならない。

### 期限切れ・接続中の無効化

| 事象 | 扱い |
|---|---|
| 券が期限切れ（60秒超） | `connection` で `close(4003, "ticket expired")`。画面は券を取り直して再接続する |
| 券の使い回し | nonce を記憶しているので拒否。`close(4003, "ticket already used")` |
| 券の署名が違う | `close(4003, "ticket invalid")` |
| 接続中にセッションが消された | 60秒以内の定期確認で検出し、`close(4001)` |
| 接続中にセッションが7日で期限切れ | 同上。`expires > now()` を確認しているため同じ経路で落ちる |
| 定期確認がアプリ側の障害で失敗した | **接続は維持し、ログに出す**（PO判断）。ただし無条件では維持しない。下記 |
| ws-server が再起動した | 全接続が切れ、画面が券を取り直して繋ぎ直す |
...
| 連続失敗 | 経過時間の目安 | 扱い |
|---|---|---|
| 1〜4回 | 〜4分 | **何もしない。** ログに1行出すだけ |
| 5回（`VERIFY_FAIL_WARN`） | 5分 | ログを警告に上げる。**新しい接続は今までどおり受ける**（券の検証はHMACだけででき、アプリに依存しないため） |
| **10回（`VERIFY_FAIL_CUT`）** | **10分** | **認証済みの接続を全て `close(4001)` で切る。** 以後、確認が1回成功するまで、券の検証を通った接続も**見るだけ扱い**にする |
...
- `presence` マップに**入れない**。よって `presenceList()` に出ない = 他の人から見えない。
  **見るだけの接続が何本増えても、村の人数は増えない**
- 受け取るのは `presence.list` の配信のみ
- **画面から送られてきたメッセージは、種別を問わず全て捨てる**
  （`VIEWER_ALLOWED` の判定より前に、`ws.authed !== true` で落とす）
- `TENKO_PUBLIC_VIEW=0` のときは、券の無い接続を `close(4004)` で拒否する
...
- **4001 / 4003 を受けたら、券を取り直して繋ぎ直す。** 券は使い捨てなので、古い券での再試行は必ず失敗する
- 失敗のたびに間隔を倍にする（500ms → 1s → 2s …、上限30秒）。
  空けないと、セッションが本当に無効なときに `/api/ws-ticket` への要求が際限なく増える
- `1006`（ハンドシェイクで切れた・回線が落ちた）も同じ経路で扱う。
  原因が判別できないため、**常に新しい券で試す**
- 券の取得に失敗したら、**見るだけの接続に落とす**（券なしで繋ぐ）。
  村が見えなくなるより、動かないが見える方がよい
- **未ログインの人は券を取りに行かない**（401 を無駄に踏まない）

### デモ用の利用者を、接続なしで村に出す

- ws-server は `/api/demo-presence` を60秒ごとに取得する
- `presenceList()` は「実際の接続から作った一覧」＋「デモ用の一覧」を結合して返す
- **デモ用の利用者は券の検証の対象外**である。接続を持たず、ws-server の内側で足す経路であり、
  外から名乗れる経路ではない
- 部屋の人数（`roomCounts`）にはデモ用も数える
...
- 2026-08-13 **6節を、スパイク `spike/ws-auth-01` の実測にもとづいて全面的に書き直した。**
  方式（入場券を `Sec-WebSocket-Protocol` で渡す）は変えていない。**書き直す前の記述に誤りが3つあった。**

  | # | 誤っていた記述 | 実測 | 直した内容 |
  |---|---|---|---|
  | 1 | 「base64url」とだけ書いていた | パディング（`=`）が1文字でも入ると、**ブラウザが `new WebSocket` の時点で `SyntaxError` を投げ、サーバには何も届かない** | 「**パディングを付けないこと**」を要件として明記。サーバ側で救えないことも書いた |
  | 2 | 「期限切れなら `close(4003)`」とだけ書いていた（ハンドシェイクで断る実装になりうる） | `handleProtocols` で `false` を返すと、ブラウザに届くのは **1006** で、4003 は届かない。さらに **`ws` はサーバ側の `connection` を発生させる**（拒否したはずの接続が存在する） | 「**ハンドシェイクで断らない**」を明記。判定は覚えるだけにし、分岐を `connection` に集める形に書き直した |
  | 3 | 「応答の値を誤ると接続できない」と書いていた | **券を含む値を返しても普通に接続できる**（`ws.protocol` が券になるだけ）。接続できなくなるのは「何も返さない」場合だけ | 券を返さない理由を「壊れるから」から「**券が応答ヘッダに載るから**」に差し替えた（券をURLに載せない理由と同じ） |

  誤り3は結論（`tenko.v1` だけを返す）自体は正しかったが、**理由が誤っていた。**
  誤った理由で正しい結論を守っている状態は、次に誰かが「別に繋がるじゃないか」と気づいた瞬間に崩れる。

  あわせて、PO判断（`/api/ws-verify` と `/api/demo-presence` を1本にまとめない。失敗時の扱いが逆であるため）と、
  再接続の方針（券を取り直して1回・間隔を倍にする・取れなければ見るだけに落とす）を6節に取り込んだ。
